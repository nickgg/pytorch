...
BoundsInfo mergeTensorAccesses(const BoundsInfo& unmerged) {
  ...
  for (auto& pair : unmerged) {
    std::vector<TensorAccessBoundsInfo>& res_vec = res[pair.first];
    for (const auto& v : pair.second) {
      ...
      for (auto& v2 : res_vec) {
        ...
        assert(v.start.size() == v.stop.size());
        assert(v2.start.size() == v2.stop.size());
        assert(v.start.size() == v2.start.size());
        ...
        for (unsigned int i = 0; i < v.start.size(); ++i) {
          ...
          auto pair =
              rangeOverlap(v.start[i], v.stop[i], v2.start[i], v2.stop[i]);
